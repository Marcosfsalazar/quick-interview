...
<responseInstruction>
Faça uma resposta em um objeto JSON válido. A resposta deve estar formatada apenas como JSON, sem nenhum texto adicional, markdown ou code fences.
</responseInstruction>
<responseType>
{
  "general_report": "",
...
4. Com tudo isso em mente você ira ler cada uma das *questions*, e ira avaliar cada *Question* individualmente, levando em conta o campo *evaluate* (caso ele exista na Question), que determina o que o empregador espera com aquela questão. Reflita sobre a questão e escreva um relatório completo da resposta do usuário para o empregador, dando uma nota de 1 a 10 com base na *job_description*, no *aditionalContext* (caso ele exista) e no *evaluate*(caso ele exista para a pergunta em questão), escreva também um feedback para o usuário, primeiro elogiando os pontos positivos da resposta dele e depois fornecendo um feedback sobre de que forma ele poderia ter melhorado/incrementado a resposta dele e da resposta dele em geral, sem fornecer informações sensiveis sobre a vaga para o usuário.
